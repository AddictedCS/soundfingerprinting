namespace SoundFingerprinting.Tests.Unit.Query
{
    ...
    public class ResultEntryConcatenatorTest
    {
        ...
        private const QueryPathReconstructionStrategyType QueryPathReconstructionStrategyType = SoundFingerprinting.LCS.QueryPathReconstructionStrategyType.MultipleBestPaths;

        [Test]
        public void ReturnNullWhenBothEntriesAreNull()
        {
            var concatenator = new ResultEntryConcatenator(QueryPathReconstructionStrategyType, loggerFactory, false);
        ...
        public void ShouldMergeMatchesThatOverlap()
        {
            ...
            // still a little controversial that query matches 100% in both, it is just a track overlap that causes it to decrease
            Assert.AreEqual(0.75d, result.QueryRelativeCoverage, 0.01);
            Assert.AreEqual(20, result.QueryLength, 0.01);
            Assert.AreEqual(0.5d, result.TrackRelativeCoverage, 0.01);
            AssertCoverageOrder(result.Coverage);
        }
        ...
        public void ShouldBeAbleToConcatenateFromBothSides()
        {
            ...
            var concatenator = new ResultEntryConcatenator(QueryPathReconstructionStrategyType, loggerFactory, autoSkipDetection: true);
        ...
        [Test(Description = "A sporadic match can create a result entry that will have query gaps but will not have track gaps")]
        public void ShouldNotConcatenateSinceMatchIsSporadic()
        {
            var left  = CreateEntry(queryOffset: 0, trackOffset:  0, matchLength: 25, queryLength: 60, trackLength: 30);
            var right = CreateEntry(queryOffset: 0, trackOffset: 25, matchLength: 5, queryLength: 60, trackLength: 30);  // sporadic match, should be ignored

            var concatenator = new ResultEntryConcatenator(QueryPathReconstructionStrategyType, loggerFactory, autoSkipDetection: true);
            
            var result = concatenator.Concat(left, right); 
            CollectionAssert.IsEmpty(result.Coverage.QueryGaps.Where(_ => !_.IsOnEdge));
        }
        
        [Test]
        [Ignore("This test enters in direct contradiction with ShouldNotConcatenateSinceMatchIsSporadic, which has proved to generate issues in real-world usage." +
                "Ideally, we should emit multiple ResultEntry matches from the concatenator, but we do not have time right now for a proper refactoring that will do it.")]
